...
//weather API based on the city or town entered. 
...
function displayMarkers(results, status) {
  if (status === google.maps.places.PlacesServiceStatus.OK) {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      shopList += `<button class=button1>${results[i].name} ${results[i].formatted_address}</button>`;
    };
...
function renderForm() {
  return `
      <form action="#" class="js-search-form">
        <label for="query"><h2>Enter your city/town name.</h2></label>
        <input type="text" id="query" class="js-query" placeholder="(Example:) Nakuru" required>
